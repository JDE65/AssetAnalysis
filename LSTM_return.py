...
seq_length = 50                     ### number of days for enriching the LSTM
...
proportionTrain = 0.98

modeRC = 'class'                 ### 'class' = classification  |  'regr' = regression  
class_trig = 0.0001                   ### trigger for investment decision for classification
regr_trig = 0.0001                   ### trigger for investment decision for regression
if modeRC == 'regr':
...
(X_train, y_train), (X_test, y_test), (res_train, res_test) = LSTM_train_test_return(Xs, ys, nn_start, nn_size, proportionTrain, modeRC)
...
layer_dense3 = Dense(units= layer_3, activation = 'relu')
...
layer_conv1D = keras.layers.Conv1D(filters = 32, kernel_size= 3, activation = 'relu')
layer_output = Dense(units = 1)

# Model architecture 
model.add(layer_conv1D)
model.add(layer_LSTMstack1)
model.add(layer_LSTMstackend)
model.add(layer_drop)
model.add(layer_dense3)
